...
### 5. 已知问题：`message` 工具调用报错
- **现象**：当 Agent 尝试通过 `message` 工具主动发送文件或消息时，可能会报错 `Outbound not configured`。
- **原因**：Moltbot 核心在某些版本中可能未正确识别插件导出的 `outbound` 接口。
- **解决方案**：请优先使用**对话回复**的方式。在对话中要求机器人发送文件（例如：“把 xxx 发给我”），机器人会通过标准回复通道发送附件，这是完全正常的。
